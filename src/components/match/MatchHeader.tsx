...
import { formatEventDate, getMatchStatus } from '../../lib/utils';
    ...
    return (
        <div className="bg-bg-surface rounded-t-xl overflow-hidden border-b-card">
            <div className="flex items-center gap-3 px-4 py-3 border-b border-border-primary">
                ...
                <span className="text-sm text-text-white font-medium">{event.strLeague}</span>
            </div>

            <div className="flex items-center justify-between px-6 py-6 md:px-12">
                <div className="flex flex-col items-center gap-2 min-w-[80px]">
                    <div className="relative">
                        <TeamBadge src={event.strHomeTeamBadge} alt={event.strHomeTeam} size="lg" />
                        {hasScore && parseInt(event.intHomeScore!) > parseInt(event.intAwayScore!) && (
                            <div className="absolute -top-1 -right-1 w-5 h-5 bg-green-accent rounded flex items-center justify-center">
                                <span className="text-[10px] font-bold text-white">{event.intHomeScore}</span>
                            </div>
                        )}
                    </div>
                ...
                <div className="flex flex-col items-center gap-1">
                    <span className="text-[10px] text-text-secondary font-medium">{dateFormatted}</span>
                    {hasScore ? (
                    ...
                    {status === 'finished' && (
                        <Badge variant="red" size="md">Finished</Badge>
                    )}
                    {status === 'live' && (
                    ...
                    {status === 'halftime' && (
                        <Badge variant="orange" size="md">Halftime</Badge>
                    )}
                    {status === 'upcoming' && (
                        <Badge variant="gray" size="md">{event.strTime?.substring(0, 5) || 'TBD'}</Badge>
                    )}
                </div>
                ...
                <div className="flex flex-col items-center gap-2 min-w-[80px]">
                    <div className="relative">
                        <TeamBadge src={event.strAwayTeamBadge} alt={event.strAwayTeam} size="lg" />
                        {hasScore && parseInt(event.intAwayScore!) > parseInt(event.intHomeScore!) && (
                            <div className="absolute -top-1 -right-1 w-5 h-5 bg-red-accent rounded flex items-center justify-center">
                                <span className="text-[10px] font-bold text-white">{event.intAwayScore}</span>
                            </div>
                        )}
                    </div>
                    <span className="text-xs md:text-sm font-semibold text-text-primary text-center leading-tight">
