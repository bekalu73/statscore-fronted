...
type DualEvent = { type: 'dual_event'; data: [TimelineEvent, TimelineEvent]; minute: string };
...
function groupTimelineItems(items: TimelineItem[]): GroupedItem[] {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (
            ...
            const [home, away] = curr.data.side === 'home'
                ? [curr.data, next.data]
                : [next.data, curr.data];
            result.push({ type: 'dual_event', data: [home, away], minute: curr.data.minute });
            i++;
        } else {
            result.push(curr);
        }
                ...
                <div className="flex flex-col gap-2">
                    {grouped.map((item, idx) => {
                        ...
                        if (item.type === 'dual_event') {
                            return <DualEventRow key={`dual-${idx}`} minute={item.minute} homeEvent={item.data[0]} awayEvent={item.data[1]} />;
                        }
...
function MinuteBadge({ minute, isGoal }: { minute: string; isGoal: boolean }) {
    return (
        <div className={`flex items-center justify-center min-w-[36px] h-[22px] rounded-full text-xs font-normal px-5 py-0.5 z-10 ${
            isGoal ? 'bg-secondary text-bg-surface' : 'bg-bg-muted text-text-white/90'
        }`}>
            {minute}
        </div>
    );
...
function PlayerInfo({ name, detail, align }: { name: string; detail?: string; align: 'left' | 'right' }) {
    return (
        <div className={`text-${align}`}>
            <div className="text-sm font-medium text-text-white leading-tight">{name}</div>
            {detail && <div className="text-xs text-text-white/40 leading-tight">{detail}</div>}
        </div>
    );
}
...
function TimelineEventRow({ event }: { event: TimelineEvent }) {
    ...
    return (
        <div className="relative flex items-center w-full min-h-[32px] z-10">
            <div className={`flex-1 flex items-center gap-2.5 justify-end pr-3 ${!isHome ? 'invisible' : ''}`}>
                {isHome && <><PlayerInfo name={event.playerName} detail={event.assistOrDetail} align="right" /><EventIcon type={event.type} /></>}
            </div>

            <div className="flex items-center justify-center w-[100px] flex-shrink-0 gap-1">
                {isHome ? connector : spacer}
                <MinuteBadge minute={event.minute} isGoal={isGoal} />
                {!isHome ? connector : spacer}
            </div>

            <div className={`flex-1 flex items-center gap-1.5 justify-start pl-3 ${isHome ? 'invisible' : ''}`}>
                {!isHome && <><EventIcon type={event.type} /><PlayerInfo name={event.playerName} detail={event.assistOrDetail} align="left" /></>}
            </div>
        </div>
    );
}
    ...
    return (
        <div className="relative flex items-center w-full min-h-[48px] z-10 gap-3">
            <div className="flex-1 flex items-center justify-end gap-3">
                <PlayerInfo name={homeEvent.playerName} detail={homeEvent.assistOrDetail} align="right" />
            ...
            <div className="flex-1 flex items-center justify-start gap-3">
                ...
                <PlayerInfo name={awayEvent.playerName} detail={awayEvent.assistOrDetail} align="left" />
            </div>
        </div>
    );
}
...
function EventIcon({ type }: { type: TimelineEvent['type'] }) {
    switch (type) {
        case 'goal':
            return <div className="flex items-center justify-center rounded-full px-1"><SoccerBallIcon fill='#00FFA5' className='size-4' /></div>;
        case 'yellow_card':
