...
function LiveIndicator() {
    return <div className="w-4 h-0.5 bg-secondary/10 rounded mt-1 overflow-hidden loader" />;
}

function TeamRow({ badge, name, redCards, tag, aggScore, score, hasScore }: {
    badge: string | null;
    name: string;
    redCards?: number;
    tag?: string;
    ...
    return (
        <div className="flex items-center gap-2.5">
            ...
            {!!redCards && redCards > 0 && (
                <div className="w-2 h-3 bg-red-accent rounded-sm flex-shrink-0 animate-pulse" title={`${redCards} Red Card(s)`} />
            )}
            {tag && (
            ...
            {aggScore && (
                <span className="text-xs text-text-muted mr-1 tabular-nums">[{aggScore}]</span>
            )}
            {hasScore && (
                <span className="text-sm font-bold text-text-primary tabular-nums w-5 text-right">{score}</span>
            )}
        </div>
    );
}
...
export default function MatchCard({ event }: MatchCardProps) {
    ...
    return (
        ...
        >
            <div className={`flex flex-col items-center justify-center w-[56px] md:w-[65px] flex-shrink-0 border-l-4 ${getStatusColor(status)}`}>
                <span className={`text-sm font-medium mr-2 ${getStatusTextColor(status)}`}>{statusText}</span>
